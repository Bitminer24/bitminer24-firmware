...
vollständig ausgerollt; sechs Compiler-Generationen bringen daran nichts.
Damit ist die Hoffnung „SW-Kernel hebt uns auf 340-350 kH/s" vom Tisch,
und die Vermutung, NMMiners 384 kH/s kämen aus einem schnelleren
SW-Anteil, ebenfalls unwahrscheinlich.

Nebenbefund: Zwei SW-Kerne unter Volllast erzeugen 58-62 °C, also **mehr**
Wärme als die 52-55 °C der 1.x mit HW-Werk — bei einem Viertel der
...
## 27.07.2026 — Hardware-SHA-Werk unter IDF 5.5 (gelöst)
...
Text- und H-Register sind beide als rohe little-endian CPU-Wörter
zugänglich, aber ihre Daten bedeuten Verschiedenes:
...
- Die portable SHA-Referenz stellt einen Midstate als normale FIPS-u32 dar.
  Das H-Registerlayout erwartet diese Wörter byteverdreht.
- Gelesene H-Register dürfen für den Digest nicht nochmals als big-endian
...
Der Geräte-Boot-Selbsttest meldet danach:
...
Nach zusätzlichen 45 Sekunden standen 441 hardwareseitige
16-Bit-Filtertreffer und null Abweichungen. Temperatur auf dem reinen
Dauerlast-Prüfstand: 59-60 °C.

### Was daraus folgt

Der IDF-5.5-Umstieg ist für den Mining-Kern nicht mehr
leistungsneutral, sondern liegt im reproduzierbaren Prüfstand etwa
8-14 kH/s über der 1.x-Gesamtreferenz. Mehr Takt wurde dafür nicht
verwendet; der ESP32-S3 läuft weiterhin mit 240 MHz.

Die 59-60 °C des pausenlosen Prüfstands sind korrekt, aber noch kein
...
thermisch unkontrollierte Optimierung. SHA-DMA bleibt ein späteres,
zeitlich begrenztes Experiment und ist keine Voraussetzung mehr für 2.0.

## 27.07.2026 — Nativer Job-/Miner-Pfad mit smarter SW-Dosierung

Der Messheader wird nicht mehr direkt im Benchmark erzeugt. Er durchläuft
jetzt den späteren Produktpfad:
...
| share-fähige Treffer | 21 |
| Abweichungen | **0** |

Die schwankenden 1-s-SW-Werte 28,7/32,8 kH/s sind Chunkgrenzen der
Duty-Regelung, kein instabiler Kernel. Über das Messfenster liegt die
Gesamtleistung knapp über 300 kH/s.

Thermische Einordnung: 300+ ist ohne Übertaktung erreicht, aber die native
IDF-5.5-Sensormessung liegt mit 60-61 °C über den 52-55 °C der
...
mit WiFi, Display und Gehäuse im Soak-Test festgelegt.

## 27.07.2026 — Produkt-RC1/RC2 auf dem Testgerät

Der feste Prüfstand wurde durch den nativen Produktpfad ersetzt und als
`2.0.0-rc1` geflasht und danach um die native Fünf-Seiten-UI zum RC2
...
Ohne gespeicherte Konfiguration startet der WPA2-Setup-AP mit HTTP-Portal.
...
Displaybetrieb, Temperaturregelung und den mehrstündigen Soak einschließen.

RC2 ergänzt einen nativen 20-ms-Tastentask, PWM-Backlight mit 130/255,
Hintergrundmetriken für Preis/Netz/Solo und getrennte TLS-Zeitfenster.
...
Speed-Doku von Espressif nennt für QIO gegenüber DIO nahezu doppelte
...
genau einmal und liest `sdkconfig.defaults` danach nie wieder. Änderungen
an den Vorgaben wirken erst, wenn die Datei gelöscht wird. Genau daran
war eine früher gesetzte Optimierungsstufe stillschweigend gescheitert.
`tools/measure.sh` löscht sie deshalb vor jedem Lauf.

Je 150 s im laufenden Betrieb, gleiche Auswertung:

| Variante | Median | Mittel | Einbrüche | Temp |
|---|---|---|---|---|
...
Erklärung, und sie war vorhersagbar: Der Hot-Loop liegt im IRAM und wird
vom Peripheriebus zum SHA-Werk begrenzt, nicht vom Befehlsnachschub aus
dem Flash. Compiler- und Flash-Einstellungen können daran nichts ändern.

Behalten wird trotzdem `-Os` + QIO: der Binärstand fällt von 52,8 % auf
50,3 % des App-Bereichs, der Start wird kürzer, und Nachteile sind keine
gemessen worden.

Bleibende Erkenntnis: Wer hier mehr Leistung sucht, muss an das SHA-Werk
(DMA-Modus), nicht an die Übersetzung.
...
Digest lesen, zweiten SHA rechnen), einmal über Register und einmal über
`esp_sha_dma()`, je 2000 Durchläufe mit dem CPU-Taktzähler:
...
**DMA ist gut viermal langsamer.** Der Grund: DMA spielt seine Stärke nur
bei vielen Blöcken am Stück aus. Unsere Kette braucht zwischen den zwei
Blöcken den Digest, also bleibt es bei einzelnen 64-Byte-Übertragungen,
und deren Einrichtung kostet ein Vielfaches von sechzehn
Registerschreibvorgängen.

Der Vergleich ist für DMA sogar noch günstig gerechnet: Die
...
Netzwerkpause (+10 kH/s). Der ESP32-S3 gibt über den Peripheriebus zum
